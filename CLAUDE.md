...
- The 15-link magazine table: transcribed directly from `Infos_Links.docx` (I parsed the raw XML myself), so the URLs are Patrick's own, not invented.

**🚩 DO NOT PUBLISH WITHOUT WRITTEN CONFIRMATION — customer name claims (raised 2026-09-07):**
The big-name customer list (Bosch, Audi, Google, Adidas, Sky, Bose, Spiegel) came from Patrick's
`Infos_Links.docx`. This doc previously argued the list was "solid" because Patrick supplied it —
that is a logical error: it establishes only that his document says so, not that it is true. Checked
2026-09-07:
- The magazine article this doc labels "Adidas" (`/magazin/filmstars`) **never says Adidas**. It says a
  form.bar shelf appeared in a commercial for "an international sports equipment maker". A prop in
  someone else's advert is not a customer relationship, and form.bar's own refusal to name the brand
  suggests they lack naming rights.
- No public evidence found for Audi, Google, Bosch, Sky, Bose or Spiegel as form.bar customers.
- form.bar's own website names **zero** client companies anywhere.
- Only Villeroy & Boch, Universität des Saarlandes and CISPA have actual project photos.
Publishing "Unternehmen, die schon auf form.bar setzen" over these names risks irreführende Werbung
...
**Black Hen "+60 % Umsatz" — republished 2026-09-09, with its source linked.** History: Sandro's
mail reports the figure; I first published it citing what I believed was the Instagram reel's
caption, which was wrong — `instagram.com/reel/DbF7s-BoyFG/` is login-walled, a fetch returns only
the app shell, and the "verbatim caption" came from a summarizer answering a leading prompt. Pulled
it, then Shivam opened the reel himself and confirmed the content (Black Hen's founders and
form.bar's own founder appear in it). It is back as the featured testimonial card, deliberately
shaped so the claim carries its evidence: the number is attributed to Black Hen's shelf at GLOBUS
Güdingen, it is **not** presented as a verbatim quote (nobody has transcribed the exact sentence),
and the card links straight to the reel so any reader can check it.
If a verbatim quote is ever wanted instead, get the exact sentence from the video plus Black Hen's
okay to print it.
Method note that still stands: never treat a fetch-tool summary of a login-walled page as a source,
and never ask it a leading question — that is how the first version got through.
...
- "Founded 2013", "~20 employees", founder names (Nikolas Feth, Alessandro Quaranta) — plausible, used throughout the build, but I have not personally found a primary source confirming these numbers/names.
- **CORRECTED 2026-09-04:** this doc previously claimed "80+ partner workshops" and the built page repeated it. form.bar's own partner page says "über 70 Partner" — the only partner-count claim anywhere on their site (the homepage states none). 80+ was never sourced. Decision: `index.html` now states no partner count at all — a number in the hero ages badly and needs no proof there, so the copy says only "von einer Partner-Schreinerei in deiner Nähe". Use "über 70" if a count is ever needed elsewhere.
...
**Founded:** 2013
**Team:** ~20 employees
**Partner network:** über 70 partner workshops across DE, AT, CH, LU, FR, IT, NL (live-verified 2026-09-04 on form.bar/de-DE/business/partner)
...
### Do NOT
- Invent statistics — only use numbers that can be verified from the actual site or official sources (verified: über 70 partner carpentries — primary-sourced; ~20 employees and founded 2013 remain UNVERIFIED, do not present them as facts without checking)
